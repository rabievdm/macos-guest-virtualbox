...
vmname="MacOsMojave"             # name of the VirtualBox virtual machine
...
linpath="/mnt/c/temp/"	# Make sure the temp path is the same location for windows and linux (WSL: /mnt/  or Cygwin: /cygdrive/)
winpath="c:\\temp\\"  # Make sure the temp path is the same location for windows and linux (WSL or Cygwin)
...
boardid="'${boardid}'"
linpath="'${linpath}'"		#WSL or Cygwin path to temp/working directory
winpath="'${winpath}'"		#Windows path to temp/working directory
...
if [ ! -d "${linpath}" ]; then
	echo "Please verify that ${linpath} exists"
	exit
fi

#if [ -n dir ${winpath} ]; then
#	echo "Please verify that ${winpath} exists"
#	exit
#fi
...
if [ -r "${linpath}BaseSystem.vdi" ]; then
    echo "BaseSystem.vdi bootstrap virtual disk image ready."
else
    echo "Downloading BaseSystem.dmg from swcdn.apple.com"
    wget -c 'http://swcdn.apple.com/content/downloads/34/52/041-38914/dhgsi49xaudtqpbj3zibbmjy3ry9ola2rb/BaseSystem.dmg' -O "${linpath}BaseSystem.dmg" 2>/dev/tty
    if [ ! -s ${linpath}BaseSystem.dmg ]; then
...
or http://swscan.apple.com/content/catalogs/others/
   index-10.14-10.13-10.12-10.11-10.10-10.9-mountainlion-lion-snowleopard-leopard.merged-1.sucatalog'
    ...
    dmg2img "${linpath}BaseSystem.dmg" "${linpath}BaseSystem.img"
    VBoxManage convertfromraw --format VDI "${winpath}BaseSystem.img" "${winpath}BaseSystem.vdi"
    rm "${linpath}BaseSystem.dmg" "${linpath}BaseSystem.img"
fi
}

# Create the target virtual disk image:
function create_target_vdi() {
if [ -r "${linpath}${vmname}.vdi" ]; then
    echo "${linpath}${vmname}.vdi target system virtual disk image ready."
...
if [ -r "${linpath}Install${vmname}.vdi" ]; then
    ...
    VBoxManage createmedium --size=8000 \
                            --filename "${winpath}Install${vmname}.vdi" \
...
VBoxManage storageattach "${vmname}" --storagectl SATA --port 0 \
           --type hdd --nonrotational on --medium "${winpath}${vmname}.vdi"
VBoxManage storageattach "${vmname}" --storagectl SATA --port 1 \
           --type hdd --nonrotational on --medium "${winpath}Install${vmname}.vdi"
VBoxManage storageattach "${vmname}" --storagectl SATA --port 2 \
           --type hdd --nonrotational on --medium ${winpath}BaseSystem.vdi
}
...
# temporary files cleanup
    VBoxManage closemedium "${winpath}BaseSystem.vdi"
    VBoxManage closemedium "${winpath}Install${vmname}.vdi"
#    rm "${linpath}BaseSystem.vdi" "${linpath}Install${vmname}.vdi"
fi
